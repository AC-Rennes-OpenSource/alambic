...
for tag in `ls "${ALAMBIC_HOME}/opt/etl/tags/" | grep -v "${ETL_VERSION}"`
do
    logger "INFO" "Suppression de la version précédente d'ETL '${tag}"
    rm -rf "${ALAMBIC_HOME}/opt/etl/tags/${tag}";
done
