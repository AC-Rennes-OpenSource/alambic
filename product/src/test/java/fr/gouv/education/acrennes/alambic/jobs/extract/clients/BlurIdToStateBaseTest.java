...
public class BlurIdToStateBaseTest extends TestCase {
     ...
     * - Two blur identifiers are requested
     * - The two entities belongs to the same person
     * - The query is based-on information from two different entities from LDAP (could be one person with multiple AAF entities - hence multiple
     * Toutatice entities - as this person is supervisor of multiple pupils in different schools)
     * - One of the phone numbers among entities is common (the person didn't get the same phone number each time when registering to school
     * services).
     */
    @Test
    ...
    public void test8() throws AlambicException {
        ...
        Assert.assertEquals(blurId2, blurId1);
    }
