...
public class FMFunctionsTest {
        ...
        try {
            ...
            while ((strLine = br.readLine()) != null) {
                if (!strLine.matches("^\"UUID\";.+")) {
                    ...
                    entity.put("PROPERTIES", Collections.singletonList(strLine.split(";")[3]));
                    final PropertyMap properties = Fn.getProperties(entity);
                    Assert.assertEquals("Evaluation diagnostique - Lire et exploiter un graphique - niveau 3 ème - seconde",
                            ((String) properties.get("dc:title")).trim());
                    Assert.assertEquals("Les élèves répondent à un questionnaire MOODLE dans le cadre d'une évaluation diagnostique  - seconde " +
                                        "(physique chimie - thème santé) possible  en AP seconde", ((String) properties.get("unum:resume")).trim());
    ...
    public void test5() {
        ...
        Assert.assertEquals("un text avec 'guillemets gênants'", Fn.normalize("un text avec \"guillemets gênants\"", NormalizationPolicy.JSON,
                false));
    ...
    @Test
    public void test7() {
        final String RESOURCE_XML_ELEMENT = "<resource type=\"grep\" name=\"test-grep-resource\" " +
                                            "dynamic=\"true\"><input>%TEST_FILE_PATH%</input><query/></resource>";
    ...
    public void test8() {
        ...
        Assert.assertNull(Fn.unescapeXML(null));
    }
