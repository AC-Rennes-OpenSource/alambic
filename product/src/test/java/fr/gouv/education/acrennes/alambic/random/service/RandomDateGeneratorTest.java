...
public class RandomDateGeneratorTest {
	...
	public void test1() {
		try {
			List<RandomEntity> entities = rg.getEntities("{\"blurid\":\"1\",\"count\":1,\"lowerYear\":\"2017\",\"upperYear\":\"2017\"}", "PROCESS_TESTU", UNICITY_SCOPE.PROCESS);
	...
	public void test2() {
		try {
			List<RandomEntity> entities = rg.getEntities("{\"blurid\":\"1\",\"count\":1,\"lowerYear\":\"1973\",\"upperYear\":\"1977\"}", "PROCESS_TESTU", UNICITY_SCOPE.PROCESS);
			Assert.assertTrue(1 == entities.size());
			Map<String, List<String>> sb = RandomGeneratorService.toStateBaseEntry(entities.get(0));
			Date d = new Date(Long.parseLong((String) sb.get("timestamp").get(0)));
			DateFormat f = new SimpleDateFormat("dd-MM-yyyy");
