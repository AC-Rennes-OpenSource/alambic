...
public class StateBaseToFileByFtl extends AbstractDestination {
    ...
    public StateBaseToFileByFtl(final CallableContext context, final Element job, final ActivityMBean jobActivity) throws AlambicException {
        ...
        String outputEncoding = context.resolveString((StringUtils.isNotBlank(job.getChildText("output-charset")))
                                                      ? job.getChildText("output-charset")
        ...
        try {
            ...
            root.put("Math", staticModels.get("java.lang.Math"));
            ...
            if (xmlFiles != null) {
                for (final Element xmlFile : xmlFiles) {
                    if (StringUtils.isNotBlank(xmlFile.getText())) {
                        root.put(xmlFile.getAttributeValue("name"),
                                freemarker.ext.dom.NodeModel.parse(new File(context.resolvePath(xmlFile.getText())), false, true));
                    }
                }
            }
