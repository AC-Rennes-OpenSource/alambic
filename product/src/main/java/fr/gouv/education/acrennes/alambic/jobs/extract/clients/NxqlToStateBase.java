...
public class NxqlToStateBase implements IToStateBase {
    ...
    public NxqlToStateBase(final String uri, final String login, final String password, final boolean useES, final String schemas,
                           final float version) {
    ...
    public List<Map<String, List<String>>> getStateBase(final Documents documents) {
        ...
        while (itr.hasNext()) {
            ...
            item.put("TITLE", Collections.singletonList(document.getTitle()));
            item.put("PROPERTIES", Collections.singletonList(getSerializedProperties(document)));
            results.add(item);
        }
        return results;
    }
