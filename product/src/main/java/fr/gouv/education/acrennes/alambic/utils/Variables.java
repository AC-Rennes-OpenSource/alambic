...
public class Variables {
            ...
            try {
                ...
                for (Entry<String, String> variable : variables.entrySet()) {
                    if (variable.getKey() != null && variable.getValue() != null) {
                        tableVars.put(variable.getKey(), new String(cipherHelper.execute(CipherHelper.CIPHER_MODE.DECRYPT_MODE,
                                Base64.decodeBase64(variable.getValue()))));
                    }
                }
    ...
    private String replaceVar(String sR, List<String> history, String varToReplace, String varValue)
        ...
        if (!history.contains(replaced)) {
            ...
            throw new AlambicException("Infinite recursive loop detected while resolving the variable '" + history.get(0) + "' (resolution history " +
                                       "is '" + String.join(",", history) + "')");
        }
    }
