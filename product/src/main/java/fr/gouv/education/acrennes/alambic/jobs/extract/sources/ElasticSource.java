...
public class ElasticSource extends AbstractSource {
    ...
    private static final Pattern SCROLL_REGEX_PATTERN = Pattern.compile("^\\d+(d|h|m|s|ms|micros|nanos)$");
    private static final String SCROLL_DEFAULT_VALUE = "5m";
    ...
    public void initialize(Element sourceNode) throws AlambicException {
        ...
        if (StringUtils.isNotBlank(page)) {
            ...
            if (StringUtils.isNotBlank(scroll)) {
                ...
                if (!m.matches()) {
                    throw new AlambicException("Le paramètre <scroll> '" + this.scroll + "' ne respecte pas le format de l'API Elastic (https://www" +
                                               ".elastic.co/guide/en/elasticsearch/reference/6.8/common-options.html#time-units)");
                }
            } else {
