        ...
        try {
            ...
            for (int index = 0; index < this.structures.size(); index++) {
                ...
                if (null != attribute && 0 < attribute.size() && StringUtils.isNotBlank(attribute.get(0))) {
                    ...
                    log.warn("Skipping entity '" + GARHelper.getInstance().getStructEntityBlurId(entity) + "' as it has no attribute " +
                             "'ENTStructureUAI' (mandatory)");
                    ...
                    for (String groupe : attribute) {
                        if (StringUtils.isNotBlank(groupe)) {
                            ...
                            if (!listStructDivCodes.contains(code)) {
                                ...
                                garGroup.setGARGroupeLibelle((StringUtils.isNotBlank(libelle))
                                                             ? libelle
                                                             : code); // get the code as label if no-one is defined
                        ...
                        if (StringUtils.isNotBlank(groupe)) {
                            ...
                            if (!listStructGrpCodes.contains(code)) {
                                if (!listStructDivCodes.contains(code)) {
                                    ...
                                    jobActivity.setTrafficLight(ActivityTrafficLight.ORANGE);
                                    log.warn("Entity '" + GARHelper.getInstance().getStructEntityBlurId(entity) + "' defines a division and group " +
                                             "with same code '" + code + "'. Both will be ignored.");
                ...
                /**
                 * The following filtering treatments will be deprecated as soon as the GARGroupe functional key becomes {UAI, code, statut}
                 * instead of simply {UAI, code}.
                 * Maybe in a future version of the grammar.
                 *
                 * Until then, groups and division sharing the same code must be excluded from the export. Moreover, the references to the excluded
                 * divisions must be removed
                ...
                transaction.begin();
                Query emQuery = em.createQuery("SELECT se FROM StaffEntity se WHERE se.primaryKey.uai = '" + ENTStructureUAI + "' AND se.type = " +
                                               ":type_parameter");
                ...
                for (StaffEntity enseignant : enseignants) {
                    ...
                    for (EnseignementEntity enseignement : enseignant.getEnseignements()) {
                        ...
                        if (EnseignementEntity.ENSEIGNEMENT_TYPE.CLASSE_MATIERE.equals(enseignement.getType()) || EnseignementEntity.ENSEIGNEMENT_TYPE.GROUPE_MATIERE.equals(enseignement.getType())) {
                            // Control the overall consistency of divisions & groups against structure definitions (GAREnsGroupeMatiere,
                            // GAREnsClasseMatiere & GARPersonGroupe Versus GARGroupeCode)
                            ...
                            if (validStructGrpCodesList.contains(groupCode) || validStructDivCodesList.contains(groupCode)) {
                                if (EnseignementEntity.ENSEIGNEMENT_TYPE.GROUPE_MATIERE.equals(enseignement.getType())) {
                                    if (validStructGrpCodesList.contains(groupCode)) {
                                        ...
                                        if (!notEmptyGrpDivCodes.contains(groupCode)) {
                                            notEmptyGrpDivCodes.add(groupCode);
                                        }
                                    } else {
                                        jobActivity.setTrafficLight(ActivityTrafficLight.ORANGE);
                                    ...
                                    if (validStructDivCodesList.contains(groupCode)) {
                                        ...
                                        jobActivity.setTrafficLight(ActivityTrafficLight.ORANGE);
                                        log.warn("Teacher entity (ENTPersonUid=" + enseignant.getPrimaryKey().getUuid() + ") references a division " +
                                                 "'" + groupCode + "' that actually is defined as a group within the structure '" + ENTStructureUAI + "'");
                                    }
                                }
                            } else {
                ...
                transaction.begin();
                emQuery = em.createQuery("SELECT se FROM StaffEntity se WHERE se.primaryKey.uai = '" + ENTStructureUAI + "' AND se.type = " +
                                         ":type_parameter");
                ...
                for (StaffEntity student : students) {
                    for (EnseignementEntity enseignement : student.getEnseignements()) {
                        ...
                        if (EnseignementEntity.ENSEIGNEMENT_TYPE.CLASSE_MATIERE.equals(enseignement.getType()) || EnseignementEntity.ENSEIGNEMENT_TYPE.GROUPE_MATIERE.equals(enseignement.getType())) {
                            String groupCode = enseignement.getDivOrGrpCode();
                            // Control the overall consistency of divisions & groups against structure definitions (GARPersonGroupe Versus
                            // GARGroupeCode)
                            if (validStructGrpCodesList.contains(groupCode) || validStructDivCodesList.contains(groupCode)) {
