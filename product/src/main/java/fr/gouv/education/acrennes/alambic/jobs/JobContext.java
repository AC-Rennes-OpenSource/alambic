...
public class JobContext implements CallableContext {
    ...
    public JobContext(final String executionPath, final Document document, final Variables variables, final Properties configuration)
        ...
        if (null != configuration && !configuration.isEmpty()) {
            ...
            log.debug(String.format("Loaded database configuration - driver:%s, url:%s, login:%s", this.variables.getHashMap().get(ETL_JDBC_DRIVER)
                    , this.variables.getHashMap().get(ETL_JDBC_URL), this.variables.getHashMap().get(ETL_JDBC_LOGIN)));
        }
    }
