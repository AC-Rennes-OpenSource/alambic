...
public class StateBaseToLdapDelete extends AbstractDestination {
    ...
    public StateBaseToLdapDelete(final CallableContext context, final Element destinationNode, final ActivityMBean jobActivity)
            ...
            try {
                ...
                if ((null != item.get(rdnAttributeName)) && (!item.get(rdnAttributeName).isEmpty())) {
                    ...
                    if (StringUtils.isNotBlank(rdn)) {
                        ...
                        log.error("The following entry specifies an empty '" + rdnAttributeName + "' attribut : '" + item);
                    }
                } else {
                    jobActivity.setTrafficLight(ActivityTrafficLight.RED);
