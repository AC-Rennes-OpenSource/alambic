...
public interface NormalizationToolbox {
	 ...
	 */
	static final String NORMALIZE_REGEX_SHRINK_QUOTES = "'{2,}='";

	/**
	 * Remove supernumerary whitespaces associated to quotes. Example: "l '   avion" becomes "l'avion"
	 * Remove supernumerary contiguous quotes. Example: "l'''avion" becomes "l'avion"
	 */
	static final String NORMALIZE_REGEX_SHRINK_QUOTES_WITH_WS = "\\s*'+\\s*='";
	 ...
	 */
	static final String NORMALIZE_REGEX_REMOVE_QUOTES = "\\s*'+\\s*=";
	 ...
	 */
	static final String NORMALIZE_REGEX_TRIM_SPECIAL_CHARACTERS = "^\\W+|\\W+$=";
	 ...
	 */
	static final String NORMALIZE_REGEX_REPLACE_WHITESPACES_AND_QUOTES_BY_HYPHEN = "[\\s']+=-";

	/**
	 * Replace supernumerary hyphen characters. Example: "l---avion" becomes "l-avion"
	 */
	static final String NORMALIZE_REGEX_SHRINK_HYPHENS = "-{2,}=-";

	/**
	 * Remove brackets. Example: "les oiseaux (bleus)" becomes "les oiseaux bleus"
	 */
	static final String NORMALIZE_REGEX_REMOVE_BRACKETS = "\\(|\\)=";

	/**
	 * Remove supernumerary whitespaces. Example: "les    oiseaux" becomes "les oiseaux"
	 */
	static final String NORMALIZE_REGEX_SHRINK_WHITESPACES = "(\\S+)\\s{2,}(\\S+)=$1 $2";

	/**
	 * Gather separated words. Example: "plate -   forme" becomes "plate-forme"
	 */
	static final String NORMALIZE_REGEX_GATHER_WORDS = "(\\w+)\\s*([^\\w\\s]+)\\s*(\\w+)=$1$2$3";

	/**
	 * Replace non-word characters by hyphen. Example: "Les 3: (graces) " becomes "Les-3-graces"
	 */
	static final String NORMALIZE_REGEX_REPLACE_SPECIAL_CHARACTERS_BY_HYPHENS = "\\W+=-";
