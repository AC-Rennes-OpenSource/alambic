    ...
    private ContentType getContentType(Map<String, String> headers) throws AlambicException {
        ...
        if (null != headers && headers.containsKey(HttpHeaders.CONTENT_TYPE)) {
            ...
            if (matcher.matches()) {
                ...
                throw new AlambicException("The content type (specified from headers) '" + contentType + "' doesn't match the pattern '<mime type>;" +
                                           " charset=<charset>' (example: 'application/json; charset=UTF-8')");
            }
        }
